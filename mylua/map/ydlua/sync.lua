...
---@private
function hsync.init()
    local trig = _C.CreateTrigger()
    _J.DzTriggerRegisterSyncData(trig, "hsync", false)
    _C.TriggerAddAction(trig, function()
        local syncData = _J.DzGetTriggerSyncData()
        ...
        local callbackData = {
            triggerPlayer = _J.DzGetTriggerSyncPlayer(),
            triggerKey = key,
...
function hsync.send(key, array)
    ...
    _J.DzSyncData("hsync", hsync.mix(key, array))
end
...
function hsync.receive(key, callback)
